...
def singlematch(main_file):
	...
	keywords_file = {'pradaxa','gemcitabine','rivaroxaban', 'edoxa', 'xarelto', 'riva', 'edoxaban', 'eliquis', 'dabigatran', 'apix', 'savaysa', 'dabi', 'edox', 'apixaban', 'elliquis', 'coumadin', 'apixa', 'warfarin'}
	pos_keywords = {'continue', 'start', 'begin', 'use'}
	neg_keywords = {'discontinue', 'use', 'stop', 'no', 'longer', 'not'}
	    ...
	    with open(results_file,"a+") as finalfile:
	    	...
	    	for line in master_file:
	           if set(line.lower().split()[:-1]) & keywords_file:
	               ...
	               matched_word = process.extract(line,keywords_file,limit=1)
	               click.secho(('Match Word:: {}'.format(matched_word)),fg="yellow")
	               ...
	               result = '"Filename":"{}",\n"Matched_Sentence":"{}",\n"Label":"{}"\n'.format(master_file.name,line,label)
	               print(result)
	               writer.writerow((main_file,line.strip(),label))
	               click.secho(('Finished Task For: {}'.format(line)),fg="blue")
